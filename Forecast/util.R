mode.density <- function(x, from=quantile(x,0.15), to=quantile(x,0.85)){
   x <- x[x >= from & x <= to]
   xp <- seq(from, to, length=1000)
   a=bgk_kde(x,n=2^(ceiling(log((to-from))/log(2))),MIN=from,MAX=to)
   posdens <- stats::spline(x=a[1,],y=a[2,],xout=xp)$y
   posdens <- 1000*posdens / ((to-from)*sum(posdens))
